...
import com.zz.lib.common.tool.FileTreeAccessor;
import com.zz.lib.common.tool.FileTreeTraverseManager;
...
public final class FileUtil {
     ...
     * @return 文件相对于基准的相对路径
     */
    public static String relatedPath(File file, File base) {//将一个文件路径的rootFrom部分替换为rootTo
        String filepath=null,root1=null,root2=null;
        try {
            filepath = "*+-.,<>?'\";:]}[{|\\=+-_\0/0)9(8*&^%$#@!`~////\\\\\\\\" + file.getCanonicalPath();//用一个不可能在文件路径中出现的字符串确保替换只发生在路径开始。
            root1 = "*+-.,<>?'\";:]}[{|\\=+-_\0/0)9(8*&^%$#@!`~////\\\\\\\\" + base.getCanonicalPath();
        }catch (IOException e){
            //这个异常其实根本不会发生，getCanonicalPath()内部代码只会在发现\0字符后抛出这个异常，并没有复杂的格式检查。
            e.printStackTrace();
            exit(1);
        }
        String ret = filepath.replace(root1, "");
        if (ret.startsWith("/")){
    ...
    public static void rmRd(File file) throws AccessDeniedException {
        if(file.exists()){
            if(file.isFile()){
                if(!file.delete())throw new AccessDeniedException("删除操作被拒绝"+file.getPath());
            }
            ...
            new FileTreeTraverseManager(rootFolder).travel(new FileTreeAccessor() {
                ...
                public void accessFolder(int layer_level, File folder, List<String> result) throws AccessDeniedException {
                    ...
                    if(files==null||files.length==0){
                        ...
                        if(!success) throw new AccessDeniedException("无法删除文件夹"+folder.getPath());
                    }
                }
